...
import os, re, sys, time, json, sqlite3, logging, hashlib, subprocess, html
...
def looks_like_xtream(text):
    patterns = [
        r'(?:server|url|host)\s*[:=]\s*(https?://[^\s,;]+)',
        r'(?:user(?:name)?)\s*[:=]\s*(\S+)',
        r'(?:pass(?:word)?|key)\s*[:=]\s*(\S+)',
    ]
...
class Database:
    ...
    def save_channel(self, name, stream_url, group_title, tvg_id, tvg_name, tvg_logo, tvg_language, source_link, source_site):
        try:
            self.conn.execute('''INSERT OR IGNORE INTO channels
                (name, stream_url, group_title, tvg_id, tvg_name, tvg_logo, tvg_language, source_link, source_site)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                (name.strip(), stream_url.strip(), group_title.strip(), tvg_id.strip(),
                 tvg_name.strip(), tvg_logo.strip(), tvg_language.strip(),
                 source_link.strip(), source_site.strip()))
            self.conn.commit()
            return True
    ...
    def get_all_channels(self):
        return self.conn.execute('''
            ...
            FROM channels ORDER BY group_title, name
        ''').fetchall()
...
def is_italian_channel(name, group_title, tvg_id, tvg_name, tvg_language):
    ...
    if group_title:
        ...
        if any(x in gt for x in ['italy', 'italia', 'ita ', 'italian']):
            return True
...
def download_and_parse_m3u(session, url, db, source_site, log):
...
class SiteScraper:
    def __init__(self, name, cfg, session, db, log):
        self.name = name
        self.cfg = cfg
    ...
    def _extract_m3u_links(self, text_html, base_url, text_clean=None):
        links = []
        exclude_patterns = ['share=', '/tag/', '/category/', '/go/report', 'facebook.com', 'twitter.com', 'x.com/sharer']
    ...
    def _extract_xtream_links(self, text, base_url):
        ...
        for line in text.splitlines():
            line = line.strip()
            if not line:
                if current_server and current_user and current_pass:
            ...
            if user:
                current_user = user.group(1); continue
            pwd = re.search(r'(?:pass(?:word)?|key)\s*[:=]\s*(\S+)', line, re.I)
            if pwd:
                current_pass = pwd.group(1); continue
        if current_server and current_user and current_pass:
            srv = current_server.rstrip('/')
            usr = current_user.strip()
            pw = current_pass.strip()
...
class BloggerScraper(SiteScraper):
    ...
    def scrape(self):
        ...
        for i, post_url in enumerate(post_urls, 1):
            ...
            all_links = m3u_links + xtream_links
            if all_links:
                self.log.info('    Link M3U/Xtream trovati: %d', len(all_links))
                for link in all_links:
                    count = download_and_parse_m3u(self.session, link, self.db, self.name, self.log)
                    italian_count += count
                    time.sleep(0.5)
    ...
    def scrape(self):
        ...
        for i, post_url in enumerate(post_urls, 1):
            ...
            for btn in content_el.select('a.wp-block-button__link, a[class*="button"], a[class*="btn"]'):
                href = btn.get('href', '')
                if href and not any(x in href for x in ['facebook', 'twitter', 'whatsapp', 'telegram', 'mailto', 'share=', 'x.com/sharer']):
...
def main():
    ...
    for name, site_cfg in SITE_CFG.items():
        ...
        if val and val.lower() not in ('false', '0', 'no', 'off'):
            if site_cfg['type'] == 'blogger':
                scrapers.append(BloggerScraper(name, site_cfg, session, db, log))
            elif site_cfg['type'] == 'wordpress':
                scrapers.append(WordPressScraper(name, site_cfg, session, db, log))
    ...
    log.info('  Canali italiani totali: %d', stats['channels'])
    ...
    if stats['channels'] > 0:
        ...
        log.info('File M3U generato: %s (%d canali, %.1f KB)',
